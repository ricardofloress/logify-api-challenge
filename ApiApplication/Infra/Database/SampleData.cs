using ApiApplication.Database.Entities;
using Microsoft.AspNetCore.Builder;
using Microsoft.Extensions.DependencyInjection;
using System;
using System.Collections.Generic;
using System.Linq;
using Microsoft.EntityFrameworkCore;

namespace ApiApplication.Database
{
    public class SampleData
    {
        public static void Initialize(IApplicationBuilder app)
        {
            using var serviceScope = app.ApplicationServices.GetRequiredService<IServiceScopeFactory>().CreateScope();
            var context = serviceScope.ServiceProvider.GetService<CinemaContext>();
            context.Database.EnsureCreated();

            var auditorium = new AuditoriumEntity
            {
                Id = 1,
                Showtimes = new List<ShowtimeEntity>
                {
                    new ShowtimeEntity
                    {
                        Id = 1,
                        SessionDate = new DateTime(2023, 1, 1),
                        Movie = new MovieEntity
                        {
                            Id = 1,
                            Title = "Inception",
                            ImdbId = "tt1375666",
                            ReleaseDate = new DateTime(2010, 01, 14),
                            Stars = "Leonardo DiCaprio, Joseph Gordon-Levitt, Ellen Page, Ken Watanabe"
                        },
                        AuditoriumId = 1,
                    }
                },
                Seats = GenerateSeats(1, 28, 22)
            };

            context.Auditoriums.Add(auditorium);

            context.Auditoriums.Add(new AuditoriumEntity
            {
                Id = 2,
                Seats = GenerateSeats(2, 21, 18)
            });

            context.Auditoriums.Add(new AuditoriumEntity
            {
                Id = 3,
                Seats = GenerateSeats(3, 15, 21)
            });

            var seat1 = auditorium.Seats?.Where(s => s.SeatNumber == 1 && s.Row == 1)?.First();
            var seat2 = auditorium.Seats?.Where(s => s.SeatNumber == 2 && s.Row == 1)?.First();
            var seat3 = auditorium.Seats?.Where(s => s.SeatNumber == 3 && s.Row == 1)?.First();
            var seat4 = auditorium.Seats?.Where(s => s.SeatNumber == 4 && s.Row == 1)?.First(); 
            var seat5 = auditorium.Seats?.Where(s => s.SeatNumber == 5 && s.Row == 1)?.First(); 
            
            context.Tickets.Add(new TicketEntity()
            {
                Id = new Guid("891e5112-e4a0-41c5-b6b5-91c6e1618a16"),
                ShowtimeId = 1,
                Seats = new List<SeatEntity>()
                {
                    seat1, seat2
                },
                Paid = false,
                CreatedTime = DateTime.Now.AddMinutes(-10)
            });
            
            context.Tickets.Add(new TicketEntity()
            {
                Id = new Guid("891e5112-e4a0-41c5-b6b5-91c6e1618a17"),
                ShowtimeId = 1,
                Seats = new List<SeatEntity>()
                {
                    seat3, seat4
                },
                Paid = false,
                CreatedTime = DateTime.Now
            });
            
            context.Tickets.Add(new TicketEntity()
            {
                Id = new Guid("891e5112-e4a0-41c5-b6b5-91c6e1618a18"),
                ShowtimeId = 1,
                Seats = new List<SeatEntity>()
                {
                    seat5
                },
                Paid = true,
                CreatedTime = DateTime.Now.AddMinutes(-1)
            });

            context.SaveChanges();
        }

        private static List<SeatEntity> GenerateSeats(int auditoriumId, short rows, short seatsPerRow)
        {
            var seats = new List<SeatEntity>();
            for (short r = 1; r <= rows; r++)
            for (short s = 1; s <= seatsPerRow; s++)
                seats.Add(new SeatEntity { AuditoriumId = auditoriumId, Row = r, SeatNumber = s });

            return seats;
        }
    }
}